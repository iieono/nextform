...
function StatsCards(props: StatsCardsProps){
  ...
  return(
    <div className="w-full pt-8 gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
      ...
      />
      <StatsCard title='Total submisions' icon={<FaWpforms className='text-orange-600 ' /> } 
        helperText="All time form submission"
        value={data?.submissions.toLocaleString() || "" }
        loading={loading}
        className='shadow-md shadow-orange-600'
      />
      <StatsCard title='Submission rate' icon={<HiCursorClick className='text-yellow-600 ' /> } 
      ...
      <StatsCard title='Bounce rate' icon={<TbArrowBounce className='text-red-600 ' /> } 
        helperText="Visits that leave vithout interacting"
...
export function StatsCard(props: StatsCardProps){
  const { title, value, helperText, loading, icon, className } = props
...
function FormCard( {form} : {form : Form}){
  return <Card>
    ...
    <CardFooter>
      ...
      {!form.published && (
        <Button asChild className="w-full mt-2 text-md gap-4 border dark:border-none border-primary" variant='secondary'>
